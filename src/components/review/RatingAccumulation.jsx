      ...
      <div className="mb-4 font-nunito">
        <div className="flex flex-col md:flex-row items-start md:items-end gap-2 mb-2">
          <div className="flex items-center">
            <FaStar className="text-yellow-500 text-2xl md:text-3xl mb-[0.5]" />
            <div className="ml-2 text-3xl md:text-4xl font-semibold">
              {averageRating.toFixed(1)}
            </div>
          </div>
          <div className="text-gray-500 hidden md:block">/</div>
          <div className="text-gray-500 hidden md:block">5.0</div>
        </div>
      ...
      <div className="mt-4 space-y-2">
        {[5, 4, 3, 2, 1].map((rating) => {
          ...
          return (
            <div
              key={rating}
              className="flex items-center font-light font-nunito text-sm md:text-base"
            >
              <span className="flex items-center gap-1 mr-2">
                {renderStars(rating)}
                <div>{rating}</div>
              </span>
              <span className="relative flex-1 h-2 md:h-3 overflow-hidden bg-gray-200 rounded-full">
                <div
